...
Given BASE_HEIGHT and SNAPSHOT_NAME, this script:
  ...
  3. Fetches morph-node/.env (or .env_hoodi) and updates snapshot name + heights.
  4. Creates a new branch, pushes both updated files, and opens a PR —
...
ENV_FILE_MAP = {
    "mainnet": "morph-node/.env",
    "hoodi":   "morph-node/.env_hoodi",
}

SNAPSHOT_VAR_MAP = {
    "mainnet": {"mpt": "MAINNET_MPT_SNAPSHOT_NAME", "zk": "MAINNET_SNAPSHOT_NAME"},
    "hoodi":   {"mpt": "HOODI_MPT_SNAPSHOT_NAME",   "zk": "HOODI_SNAPSHOT_NAME"},
}
...
# ── Env file update (in-memory) ───────────────────────────────────────────────

def apply_env_update(content: str, environment: str, snapshot_name: str,
                     deriv_height: str, l1_msg_height: str, base_height: str) -> str:
    """Update snapshot name and validator heights in a .env file."""
    is_mpt       = snapshot_name.startswith("mpt-")
    snapshot_var = SNAPSHOT_VAR_MAP[environment]["mpt" if is_mpt else "zk"]

    def replace(var, value):
        return re.sub(rf'^({re.escape(var)}=).*$', rf'\g<1>{value}', content, flags=re.MULTILINE)

    content = replace(snapshot_var,           snapshot_name)
    content = replace("DERIVATION_START_HEIGHT", deriv_height)
    content = replace("L1_MSG_START_HEIGHT",      l1_msg_height)
    if is_mpt:
        content = replace("L2_BASE_HEIGHT", base_height)
...
def main() -> None:
    ...
    try:
        ...
        if dry_run:
            print("\n[DRY RUN] Skipping README/env update and PR creation.")
            print(f"          Would update: env={environment} snapshot={snapshot_name}")
        ...
        # ── Step 2: update README + env file, push via GitHub API ────────────
        print(f"\n[2/4] Updating README via GitHub API ...")
        ...
        if env_path:
            ...
            env_content, env_blob_sha = gh_get_file(repo, env_path, token)
            updated_env = apply_env_update(
                env_content, environment, snapshot_name, deriv_height, l1_msg_height, base_height
            )
            env_commit_msg = f"snapshot: update {env_path} for {snapshot_name} ({environment})"
            gh_update_file(repo, env_path, updated_env, env_blob_sha, branch, env_commit_msg, token)
        else:
            print(f"\n[3/4] No env file mapping for {environment}, skipping.")
